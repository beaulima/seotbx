...
class VDSR(nn.Module):
    ...
    def __init__(self, num_channels=1,
        ...
        self.input_conv = seotbx.dl.nn.common.ConvBlock(input_size=num_channels, output_size=base_filter, kernel_size=self.kernel_size0,
        ...
        if self.num_residuals:
            for _ in range(self.num_residuals):
                conv_blocks.append(seotbx.dl.nn.common.ConvBlock(input_size=base_filter, output_size=base_filter, kernel_size=3,
                                                               stride=1, padding=1, norm=norm, bias=False, groups=groups,
                                                               activation=activation))
            self.residual_layers = torch.nn.Sequential(*conv_blocks)
        self.output_conv = seotbx.dl.nn.common.ConvBlock(input_size=base_filter, output_size=num_channels, kernel_size=3,
    ...
    def weight_init(self):
        for m in self.modules():
            seotbx.dl.nn.common.weights_init_kaiming(m)
